...
export default function register(server: McpServer): void {
  server.registerTool(
    ...
    async ({ code, threadContext, timeout, maxOutputChars }) => {
      ...
      // ponytail: bare `return game.Foo.Bar` → tree probe (works when nested loadstring is flaky)
      const instancePath = trimmed.match(/^return\s+(game\.[A-Za-z0-9_.]+)\s*;?\s*$/);
      if (instancePath) {
        const tree = await sendAndWait({
          ...
          maxOutputChars,
          stampClient: true,
          truncationHint: "Use get-descendants-tree with a smaller maxDepth/maxChildren.",
        });
